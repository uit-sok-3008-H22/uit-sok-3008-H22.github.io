...
library(haven)
#friberg_gronqvist_wine_data <- read_dta("http://ansatte.uit.no/oystein.myrland/SOK3008/H2020/friberg_gronqvist_wine_data.dta")
...
dframe <- wine %>% 
  ...
  summarise(quantity=sum(litre), expenditure=sum(litre*price)) %>% 
...
test %>% pivot_wider(names_from = "segm",
                     values_from = c("quantity", "expenditure", "price"))
